    ...
    <div class="lan">
      <div class="da">
        <div class="top">
          ...
          >
            <el-button type="info" round>批量导入白名单</el-button>
          </el-upload>
          <slot />
        </div>
      </div>
      <div style="width:98%; height:520px;background:rgba(255,255,255,1);box-shadow:0px 0px 13px 0px rgba(63,106,255,0.08);border-radius:3px; margin-left:14px; padding:10px;  margin-top:15px;">
        <div style="display:flex;  float:right; margin-right:19px;">
          <img
            ...
            class="img1"
          >
          <p class="bmd" @click="daochu()">导出白名单模板</p>
        </div>
        <el-table
          ref="multipleTable"
          :data="arr"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span :style="scope.row.certificateType == 0 ? 'color:black' : 'color:black'">
                {{ scope.row.certificateType == 0 ? "身份证" : "身份证" }}
              </span>
            </template>
          </el-table-column>
          <el-table-column
