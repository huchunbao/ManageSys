  ...
  <div>
    ...
    <div v-if="zh" class="tank">
      <div id="zzc">
        <div class="aa_a">
          <p class="cz">修改处警内容</p>
          <p class="camera">{{ zhfadata.source_name }} &nbsp;{{ zhfadata.alarm_type_name }}</p>
          <p class="time">位置：{{ zhfadata.alarm_address }} 告警时间： {{ zhfadata.alarm_time }}</p>
          <textarea id="content" v-model="data.xr" name="11" cols="30" rows="5" value=""></textarea>
          <input type="button" class="qxa" value="取消" @click="clickok">
          <input type="button" class="qda" value="确定" @click="content(), clickok">
        </div>
      </div>
    </div>
    <div v-if="xg" class="tanka">
      <div id="zzca">
        <div class="aa_aa">
          ...
          >
            <p>处警时间: {{ xgdata.processingTime }}</p>
            <p style="padding-left:22px;"> 处警人员: {{ xgdata.handlerName }}</p>
          </div>
        </div>
      </div>
    </div>
