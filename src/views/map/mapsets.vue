<template>
  <div class="mapsets">
    <div class="leftdiv">
      <div v-for="(item,i) in data" :key="i" :class="tabfa(i)?'list zhong':'list'" @click="addmarker(item,i)">
        <img :src="item.name=='售票处'?require('../../assets/icon/icon_shoupiao2.png'):item.icon" class="icon" alt="">{{ item.name }}
      </div>
      <!-- <div class="xin">
...
export default {
  ...
  methods: {
    addmarker(data, i) {
      ...
      // for (var y = 0; y < data.path.length; y++) {
      //   var marker = new AMap.Marker({
      //     position: data.path[y],
      //     icon: data.icon
      //   })
      //   markers.push(marker)
      // }
